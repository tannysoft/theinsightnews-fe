...
/**
 * Yoast types every post as a generic `Article`. Everything published here is
 * news, and NewsArticle is what Google's news surfaces (Top stories, News tab)
 * look for, so narrow the type. Only the Article node is touched — WebPage,
 * ProfilePage and the rest keep theirs.
 */
/** Apply `fn` to every top-level node, whatever shape the graph arrives in. */
function mapNodes(schema, fn) {
  if (!schema) return schema;
  if (Array.isArray(schema?.["@graph"])) {
    return { ...schema, "@graph": schema["@graph"].map(fn) };
  }
  if (Array.isArray(schema)) return schema.map(fn);
  return fn(schema);
}

function preferNewsArticle(node) {
  const t = node?.["@type"];
  if (t === "Article") return { ...node, "@type": "NewsArticle" };
  if (Array.isArray(t) && t.includes("Article")) {
    return { ...node, "@type": t.map((x) => (x === "Article" ? "NewsArticle" : x)) };
  }
  return node;
}

/**
 * Point the sitelinks searchbox at the search page we actually serve.
 *
 * Yoast builds the WebSite node's SearchAction from WordPress' own `/?s=`
 * query string. The Next app ignores `?s=` — the homepage renders as if
 * nothing was searched — so once the host has been rewritten the graph
 * advertises a searchbox that silently does nothing. Search lives at
 * /search?q= here. Applied key-agnostically, since Yoast has emitted the
 * template both as `target` directly and nested in an EntryPoint.
 */
const WP_SEARCH_QUERY_RE = /\/\?s=\{search_term_string\}$/;

function rewriteSearchTargets(value) {
  if (typeof value === "string") {
    return WP_SEARCH_QUERY_RE.test(value)
      ? `${SITE.url}/search?q={search_term_string}`
      : value;
  }
  if (Array.isArray(value)) return value.map(rewriteSearchTargets);
  if (value && typeof value === "object") {
    const out = {};
    for (const [k, v] of Object.entries(value)) out[k] = rewriteSearchTargets(v);
    return out;
  }
  return value;
}
...
export function yoastSchema(yoast, canonicalPath) {
  ...
  const rewritten = rewriteSearchTargets(rewriteSchema(yoast.schema, urlMap));
  return mapNodes(applySiteBranding(rewritten), preferNewsArticle);
}
